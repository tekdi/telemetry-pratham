...
class TelemetryService {
  constructor(Dispatcher, config) {
    this.config = config;
    this.dispatcher =
      this.config.localStorageEnabled === "true"
  ...
  dispatch(req, res) {
    ...
    if (
      this.config.localStorageEnabled === "true" ||
      this.config.telemetryProxyEnabled === "true"
    ) {
      if (
        this.config.localStorageEnabled === "true" &&
        this.config.telemetryProxyEnabled !== "true"
        ...
        const callback = this.getRequestCallBack(req, res);
        console.log("Callback type:", typeof callback);
        console.log("Callback:", callback);
        this.dispatcher.dispatch(message.mid, data, callback);
        ...
        return res.json(
          this.config.localStorageEnabled === "true" &&
            this.dispatcher.dispatch(
              ...
              message.mid,
              data,
              this.getRequestCallBack(req, res)
            )
        );
      } else if (
        this.config.localStorageEnabled !== "true" &&
  ...
  health(req, res) {
    if (this.config.localStorageEnabled === "true") {
      this.dispatcher.health((healthy) => {
        if (healthy) this.sendSuccess(res, { id: "api.health" });
        else
          this.sendError(res, {
            id: "api.health",
  ...
  getRequestCallBack(req, res) {
    console.log("getRequestCallBack");
    return (err, data) => {
      if (err) {
